...
export const populateSampleUsers = internalMutation({
  args: {},
  handler: async (ctx) => {
    const now = Date.now();
    const usernameUpdatedAt = now - (USERNAME_CHANGE_COOLDOWN_MS + 1);
    ...
    type SeedOnboardingStatus =
      | "new"
      | "verified"
      | "user_profile_complete"
      | "mentee_profile_setup_complete";
    ...
    const usersToInsert: Array<{
      ...
      onboardingStatus: SeedOnboardingStatus;
    }> = [];
